...
function evaluateModel(label, buildPrediction, rounds, windows) {
  ...
  for (let offset = windows; offset >= 1; offset -= 1) {
    ...
    const shouldBet = Boolean(prediction?.selectiveStrategy?.currentDecision?.shouldBet)
    const recommended = prediction?.selectiveStrategy?.currentDecision?.recommendedResult || predicted
...
const windows = Math.min(evalRounds, Math.max(rounds.length - 300, 1))

const results = [
  evaluateModel('V1', buildPredictionV1, rounds, windows),
  evaluateModel('V3', buildPredictionV3, rounds, windows),
]
